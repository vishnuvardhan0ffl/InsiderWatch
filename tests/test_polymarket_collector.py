...
def test_cache_key_differs_by_params():
    key_a = _cache_key("/trades", {"limit": 5})
    key_b = _cache_key("/trades", {"limit": 10})
    assert key_a != key_b


def test_trade_query_taker_only_has_no_hidden_default_bias():
    # WS4 trap: the *API's* default for takerOnly is true. This dataclass
    # also defaults to true, but the point is the caller sees and sets it
    # explicitly rather than it being silently absent from the params dict.
    q = TradeQuery(market="0xabc", start=0, end=100)
    assert q.taker_only is True
    q2 = TradeQuery(market="0xabc", start=0, end=100, taker_only=False)
